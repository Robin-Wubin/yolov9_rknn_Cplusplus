...
int read_image(image_buffer_t *image, unsigned char *data, int size)
{
    ...
    ret = tjDecompressHeader3(handle, data, size, &width, &height, &subsample, &colorspace);
    if (ret < 0)
    {
        printf("header file error, errorStr:%s, errorCode:%d\n", tjGetErrorStr(), tjGetErrorCode(handle));
        return -1;
    ...
    ret = tjDecompress2(handle, data, size, sw_out_buf, width, 0, height, pixelFormat, flags);
